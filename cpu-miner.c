...
bool have_getblocktemplate = true;
...
static bool submit_upstream_work(CURL *curl, struct work *work)
{
	...
	if (have_stratum) {
		...
		char ntimestr[17], noncestr[17], *xnonce2str;

		//uint64_t *pdata64=(uint64_t*)&work->data16[1];
		be64enc(&ntime, work->data64[12]);
		be64enc(&nonce, work->data64[14]);
...
start:
	...
	val = json_rpc_call(curl, rpc_url, rpc_userpass,
			    have_getblocktemplate ? gbt_req : getwork_req,
			    &err, have_getblocktemplate ? JSON_RPC_QUIET_404 : 0);
	...
	if (!have_getblocktemplate && !allow_getwork) {
	...
	if (have_getblocktemplate && allow_getwork && !val && err == CURLE_OK) {
...
static void stratum_gen_work(struct stratum_ctx *sctx, struct work *work)
{
	...
	for (i = 0; i < 8; i++)
		work->data[0 + i] = be32dec((uint32_t *)sctx->job.prevhash + i);
	for (i = 0; i < 8; i++)
			work->data[8 + i] = le32dec((uint32_t *)merkle_root + i);
	for (i = 0; i < 8; i++)
		work->data[16 + i] = be32dec((uint32_t *)sctx->job.accroot + i);


	work->data64[12] = be64dec(sctx->job.ntime);
	work->data64[13] = be64dec(sctx->job.nheight);
	//uint64_t *nonce_ptr = &work->data64[14];
	work->data16[60] = be16dec(sctx->job.version);
	...
	applog(LOG_DEBUG,"\n\nDifficulty: %f\n",sctx->job.diff);
	if (opt_debug) {
		char data_str[245], target_str[65],merkle_str[65];
		bin2hex(data_str, (unsigned char *)work->data, 122);
		...
		bin2hex(merkle_str, (unsigned char *)merkle_root, 32);
		//applog(LOG_DEBUG, "DEBUG: stratum_gen_work merkle root %s", merkle_str);
	}
}
...
static void *miner_thread(void *userdata)
{
	...
	int thr_id = mythr->id;
	struct work work = {{0}};
	uint64_t *pnonce=&work.data64[14];
	uint32_t max_nonce;
	...
	while (1) {
		...
		if (have_stratum) {
			...
			if (*pnonce >= end_nonce && !memcmp(work.data, g_work.data, 112))
				stratum_gen_work(&stratum, &g_work);
		} else {
			int min_scantime = have_longpoll ? LP_SCANTIME : opt_scantime;
			/* obtain new work from internal workio thread */
			pthread_mutex_lock(&g_work_lock);
			bool nonce_over = *pnonce >= end_nonce;
		...
		if (memcmp(work.data, g_work.data, 112)) {
			work_free(&work);
			work_copy(&work, &g_work);
			*pnonce = 0xffffffffU / opt_n_threads * thr_id;
		} else {
			(*pnonce)++; // todo
		}
		pthread_mutex_unlock(&g_work_lock);
		...
		if (*pnonce + max64 > end_nonce)
			max_nonce = end_nonce;
		else
			max_nonce = *pnonce + max64;

		hashes_done = 0;

		gettimeofday(&tv_start, NULL);
		/* scan nonces for a proof-of-work hash */
		rc = scanhash_m7hash(thr_id, work.data, work.target, max_nonce, &hashes_done);
