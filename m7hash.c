...
#define M7_MIDSTATE_LEN 112
int scanhash_m7hash(int thr_id, uint32_t *pdata, const uint32_t *ptarget, uint64_t max_nonce, unsigned long *hashes_done)
{
    ...
    uint32_t* porigNonce=&pdata[28];
    uint32_t* pnewNonce=&data[28];
    uint32_t n = *porigNonce - 1;
    const uint32_t first_nonce = *porigNonce;
        ...
        *pnewNonce = ++n;
        ...
        if (rc) {
            if (opt_debug) {
            	...
            	bin2hex(ntime_str,((unsigned char*)&data[24])+2,8);
            	bin2hex(nonce_str,(unsigned char *)pnewNonce,8);
                ...
                applog(LOG_DEBUG, "DEBUG: [%d thread] Found share!\ndata   %s\nhash   %s\ntarget %s\ntime %s\nnonce %s", thr_id,
                    data_str,
                    hash_str,
                    target_str,ntime_str,nonce_str);
            }

            *porigNonce=*pnewNonce;
