		      ...
		      int *curl_err, int flags)
{
	...
	if (!have_longpoll && want_longpoll && hi.lp_path && !have_getblocktemplate &&
...
static bool socket_full(curl_socket_t sock, int timeout)
{
	...
	bool result=select(sock + 1, &rd, NULL, NULL, &tv) > 0;
...
{
	const char *job_id, *prevhash, *coinb1, *coinb2, *version, *nbits, *ntime, *accroot,*nheight;
	...
	accroot = json_string_value(json_array_get(params, 4));

	merkle_arr = json_array_get(params, 5);
	if (!merkle_arr || !json_is_array(merkle_arr))
		goto out;
	merkle_count = json_array_size(merkle_arr);
	nheight=json_string_value(json_array_get(params,6));
	version = json_string_value(json_array_get(params, 7));
	//nbits = json_string_value(json_array_get(params, 7));
	ntime = json_string_value(json_array_get(params, 8));
	clean = json_is_true(json_array_get(params, 9));

	if (opt_debug){
		applog(LOG_DEBUG, "job_id: %s",job_id);
		applog(LOG_DEBUG, "prevhash: %s",prevhash);
		applog(LOG_DEBUG, "coinb1: %s",coinb1);
		applog(LOG_DEBUG, "coinb2: %s",coinb2);
		applog(LOG_DEBUG, "accroot: %s",accroot);
		applog(LOG_DEBUG, "merkle_count: %d",merkle_count);
		applog(LOG_DEBUG, "version: %s",version);
		applog(LOG_DEBUG, "nheight: %s",nheight);
		applog(LOG_DEBUG, "ntime: %s",ntime);
		applog(LOG_DEBUG, "clean: %s",clean?"true":"false");
	}

	if (!job_id || !prevhash || !coinb1 || !coinb2 || !version || !nheight || !ntime ||
	    strlen(prevhash) != 64 || strlen(version) != 4 ||
	    strlen(nheight) != 16 || strlen(ntime) != 16 || strlen(accroot)!=64) {
	...
	hex2bin(sctx->job.accroot, accroot, 32);
	...
	hex2bin(sctx->job.nheight, nheight, 8);
